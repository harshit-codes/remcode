...
if [[ $FILE_COUNT -gt 3 ]] && [ "$SESSION_UPDATED" != "true" ]; then
    ...
    echo ""
    echo "🖥  Follow the Session Documentation Process:"
    echo "   1. cd scripts"
    echo "   2. npm run session:template > template.json"
    echo "   3. Edit template.json with your session details"
    echo "   4. npm run session:add template.json"
    echo "   5. git add ../sessions/sessions.json"
